...
{
    register_native("show_mma_list", "native_show_mma_list");
	register_native("show_mma_config", "native_show_mma_config");
	register_native("broadcast_bgm", "native_broadcast");
} 

public native_show_mma_list(iplugins , iparms)
{
	new callid = get_param(1);
	music_showmenu(callid);
}

public native_show_mma_config(iplugins , iparms)
{
	new callid = get_param(1);
	config_showmenu(callid);
}

public native_broadcast(iplugins , iparms)
{
	new callid = get_param(1);
	new callbgm = get_param(2);
	broadcast_bgm(callid , callbgm);
}
...
show_time_bar(id, percent, Float:hold)
{
	...
	//	show_hudmessage(id, "BGM:[%s]", bar);
		ShowSyncHudMsg(id, g_hudobj, "Now Playing [%s] : [%s]", aBGM[MENU_TITLE] ,  bar);
	}
...
public broadcast_bgm(id , bgmid)
{
	...
	else
	{
		if (0 < num <= bgmCount)
		{
			...
			client_cmd(0, "mp3 stop");
			client_cmd(0, "mp3 play %s", aBGM[FILE_PATH]);
			client_print_color(0, print_chat, "^4[MMA] ^3%s: BGM START! ^2[%s]" , name ,  aBGM[MENU_TITLE]);
		}
		else
		{
			client_print_color(id, print_chat, "^4[MMA] ^3%s^2没有BGM注册在这个id  %d.", name , bgmid);
		}
	}
	return PLUGIN_HANDLED;
}
...
config_showmenu(id)
{
	new menu = menu_create("MP3 : 设置", "config_menu_handler");
	menu_additem(menu, "显示时间轴^t",				"",	 0, g_config_callback);
	menu_additem(menu, "播放列表设置^t",			"",	 0, g_config_callback);
	menu_additem(menu, "循环模式^t",				"",	 0, g_config_callback);
	menu_additem(menu, "加入时播放音乐^t",			"",	 0, g_config_callback);
	menu_additem(menu, "广播音乐",					"",	 0, g_config_callback);
	menu_additem(menu, "音量增大",					"",	 0, g_config_callback);
	menu_additem(menu, "音量减小",					"",	 0, g_config_callback);
...
public config_menu_callback(id, menu, item)
{
	...
	{
		case 0:
			menu_item_setname(menu, item, fmt("显示时间轴^t\y[%s]", ((g_config[id][SHOW_HUD] > 0) ? "开" : "关")));
		case 1:
			menu_item_setname(menu, item, fmt("播放列表模式^t\y[%s]", ((g_config[id][SHUFFLE] > 0) ? "随机" : "顺序")));
		case 2:
			menu_item_setname(menu, item, fmt("循环模式^t^t\y[%s]", ((g_config[id][LOOP] > 0) ? "开" : "关")));
		case 3:
			menu_item_setname(menu, item, fmt("加入时播放音乐^t^t\y[%s]", ((g_config[id][PLAY_ON_JOIN] > 0) ? "是" : "否")));
		case 4:
			menu_item_setname(menu, item, fmt("广播音乐^t^t\y[%s]", ((g_config[id][BROADCAST] > 0) ? "是" : "否")));
	}
	return PLUGIN_CONTINUE;
}
...
public config_menu_handler(id, menu, item)
{
	...
	switch(item)
	{
		...
		case 4:
			g_config[id][BROADCAST]		= (g_config[id][BROADCAST] > 0) ? 0 : 1;
		case 5:
		{
...
music_showmenu(id)
{
	...
	new menu = menu_create("MP3 : BGM列表",	"music_menu_handler");
	menu_additem(menu, "播放全部",	"all", 	0);
	menu_additem(menu, "停止播放",	"stop", 0);
	menu_addblank2(menu);
	for(new i = 0; i < ArraySize(g_bgm_list); i++)
	{
		ArrayGetArray(g_bgm_list, i, aBGM, sizeof(aBGM));
...
public music_menu_handler(id, menu, item)
{
	...
	else
	{
		...
		if(g_config[id][BROADCAST] == true)
		{
			broadcast_bgm(id , num + 1);
		}
		else
		{
			g_isPlaying[id][MODE] 		= 0; // single
